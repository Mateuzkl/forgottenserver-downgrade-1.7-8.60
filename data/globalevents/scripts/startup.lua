function onStartup()
    ...
    local resultId = db.storeQuery(
        "SELECT `id`, `highest_bidder`, `last_bid`, (SELECT `balance` FROM `players` WHERE `players`.`id` = `highest_bidder`) AS `balance` " ..
        "FROM `houses` WHERE `owner` = 0 AND `bid_end` != 0 AND `bid_end` < " .. os.time()
    )
    if resultId ~= false then
        repeat
    ...
    if configManager.getBoolean(configKeys.CHECK_DUPLICATE_STORAGE_KEYS) then
        ...
        for _, variableName in ipairs(variableNames) do
            ...
            if duplicates then
                ...
                logInfo(">> Checking " .. variableName .. ": " .. message)
            else
                logInfo(">> Checking " .. variableName .. ": No duplicate keys found.")
            end
        end
    end
end
